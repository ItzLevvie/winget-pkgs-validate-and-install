...
if %ERRORLEVEL% EQU 1 (
    ...
    setx path "%PATH%;C:\Users\%USERNAME%\AppData\Local\Microsoft\WindowsApps" > nul 2>&1
)
...
git -C %REPOSITORY_PATH% fetch --force upstream refs/pull/%PR_NUMBER%/head:pull/%PR_NUMBER% > nul 2>&1
if %ERRORLEVEL% EQU 128 goto :2
git -C %REPOSITORY_PATH% checkout --force pull/%PR_NUMBER% > nul 2>&1
goto :3

:3
for /f "tokens=1,2,3,4,5,6,7,8 delims=/" %%a in ('git -C %REPOSITORY_PATH% diff --name-only --diff-filter=d upstream/master...pull/%PR_NUMBER%') do (
