...
function Set-WinGetSettings {
    ...
    if (-not($WINGET_SETTINGS)) {
        if ($SID_CURRENT -eq $SID_REQUIRED) {
            Invoke-WebRequest -Uri https://github.com/ItzLevvie/winget-pkgs-validate-and-install/releases/download/20251002.1/settings.json -OutFile $env:LOCALAPPDATA\Packages\Microsoft.DesktopAppInstaller_8wekyb3d8bbwe\LocalState\settings.json
