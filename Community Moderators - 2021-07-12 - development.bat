...
if %ERRORLEVEL% == 1 (
    echo:
    git clone --depth 1 --single-branch https://github.com/microsoft/winget-pkgs %REPOSITORY_PATH% > nul 2>&1
...
git -C %REPOSITORY_PATH% fetch --depth 1 upstream refs/pull/%PR_NUMBER%/head:pull/%PR_NUMBER% > nul 2>&1
